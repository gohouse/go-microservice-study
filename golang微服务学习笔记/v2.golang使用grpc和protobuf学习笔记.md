...
## 2. 编写 protobuf 文件
- 创建 `v2/protoc/protoc.helloword.proto` 文件
```proto
syntax = "proto3";  //语法声明
...
- 生成 `protoc.helloworld.pb.go` 文件
```bash
# 手动指定目录并生成
protoc -I helloworld/ v2/protoc/protoc.helloworld.proto --go_out=plugins=grpc:helloworld
# 或者 快捷生成所有
cd v2/protoc
